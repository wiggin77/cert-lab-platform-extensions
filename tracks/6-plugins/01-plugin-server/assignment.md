...
- type: text
  contents: |-
    **Modules 2 to 5 are already done in this environment.** Every integration is
    configured and the handler in `~/handler` implements all of them: formatted
    alerts, automatic escalation, `/threat`, and the Escalate button and dialog.
    This lab does not touch that code. You work in `~/plugin` instead.
tabs:
- id: fwjib2kakmkx
  title: Mattermost
  type: service
