...
- type: text
  contents: |-
    **Modules 2 and 3 are already done in this environment.** The incoming and
    outgoing webhooks are configured, alerts arrive formatted in `~alerts`, and
    CRITICAL ones already escalate to `~incidents` on their own. Code you did not
    write is in `src/payloads/` and `src/routes/` for that reason. Build on it.
tabs:
- id: vxe10ebd8lk4
  title: Mattermost
  type: service
