...
- type: text
  contents: |-
    The automatic escalation already running in this environment catches CRITICAL
    alerts, but it carries no analyst judgement. It cannot say which systems are
    affected, who is picking it up, or what the responder already ruled out.
...
- type: text
  contents: |-
    **Modules 2 to 4 are already done in this environment.** Both webhooks and the
    `/threat` slash command are configured, and their handler code is in place.
    Alerts arrive formatted, CRITICAL ones escalate automatically, and `/threat`
    returns enrichment. Code you did not write is there for that reason.
tabs:
- id: vpi2cpciom4d
  title: Mattermost
  type: service
