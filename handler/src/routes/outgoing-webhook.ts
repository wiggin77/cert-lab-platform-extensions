...
export async function handleOutgoingWebhook(req: FastifyRequest, reply: FastifyReply): Promise<FastifyReply> {
    ...
    // TODO Module 2, step 1: reject the request unless the token matches
    ...
    // TODO Module 2, step 2: fetch the triggering post with getPost(payload.post_id) and
    ...
    // TODO Module 2, step 3: build an escalation attachment carrying the source,
    // indicator, and severity, plus permalink(payload.post_id) so the on-call team can
    // jump back to the original alert.

    // TODO Module 2, step 4: post it to config.channels.incidents with createPost().
